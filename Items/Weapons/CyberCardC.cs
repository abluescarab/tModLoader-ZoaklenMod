...
{
	public class CyberCardC : ModItem
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Cyber Card (Intelligent)");
			Tooltip.SetDefault("Passive: Homes next enemies");
		}
		...
		public override void SetDefaults()
		{
			...
			item.thrown = true;
			item.autoReuse = true;
			item.shoot = mod.ProjectileType("CyberCardC");
			item.crit = 6;
		}
		...
		public override bool CanUseItem(Player player)
		{
			...
			if(player.altFunctionUse == 2)
			{
				if(modPlayer.lastChange <= 0)
				{
					...
					int num = Item.NewItem((int)player.position.X, (int)player.position.Y, player.width, player.height, mod.ItemType("CyberCardD"), stack, false, 0, false, false);
