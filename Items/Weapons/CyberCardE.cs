...
{
	public class CyberCardE : ModItem
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Cyber Card (Vampire)");
			Tooltip.SetDefault("Passive: Returns damage as health");
		}
		...
		public override void SetDefaults()
		{
			...
			item.thrown = true;
			item.autoReuse = true;
			item.shoot = mod.ProjectileType("CyberCardE");
			item.crit = 6;
		}
		...
		public override bool CanUseItem(Player player)
		{
			...
			if(player.altFunctionUse == 2)
			{
				if(modPlayer.lastChange <= 0)
				{
					...
					int num = Item.NewItem((int)player.position.X, (int)player.position.Y, player.width, player.height, mod.ItemType("CyberCardA"), stack, false, 0, false, false);
