...
{
	public class CyberCardA : ModItem
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Cyber Card (Agile)");
			Tooltip.SetDefault("Passive: Very fast move speed");
		}
		...
		public override void SetDefaults()
		{
			...
			item.thrown = true;
			item.autoReuse = true;
			item.shoot = mod.ProjectileType("CyberCardA");
			item.crit = 6;
		}
		...
		public override bool CanUseItem(Player player)
		{
			...
			if(player.altFunctionUse == 2)
			{
				if(modPlayer.lastChange <= 0)
				{
					...
					int num = Item.NewItem((int)player.position.X, (int)player.position.Y, player.width, player.height, mod.ItemType("CyberCardB"), stack, false, 0, false, false);
